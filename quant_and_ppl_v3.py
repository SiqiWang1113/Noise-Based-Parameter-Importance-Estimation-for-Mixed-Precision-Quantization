...
def get_layer_config(model, bits_low=3, bits_high=4):
    ...
    print(f">>> 策略: 前5层和后5层的FFN用 {bits_high}bit, 其他用 {bits_low}bit\n")
    ...
    for name, module in model.named_modules():
        ...
        is_ffn = any(ffn_name in name for ffn_name in ffn_modules)
        
        if is_ffn and (layer_idx < 3 or layer_idx >= n_layers - 3):
            bits = bits_high
            high_precision_count += 1
            precision_marker = "FFN-4BIT"
        else:
            bits = bits_low
            low_precision_count += 1
            precision_marker = "3BIT"
        ...
        module_type = "FFN" if is_ffn else "ATT"
        print(f"[Layer {layer_idx:2d}] {bits}bit ({module_type}) {precision_marker:9s} - {name.split('model.layers.')[-1]}")
...
def main():
    ...
    print(f"混合精度配置: {bits_low}bit / {bits_high}bit (前5层和后5层FFN)")
    print("="*60 + "\n")
    ...
    try:
        ...
        print(f"输出目录: {out_dir}")
        print(f"配置: 前5层和后5层FFN={bits_high}bit, 其他={bits_low}bit")
        print("="*60)
    except Exception as e:
